...
get_intro_text <- function(){
  div(h3("Welcome to the MPIAE Retreat 2024 Debrief Monitor App"), 
...
impressum <- function(){
    shiny::p(
        "MPIAE Retreat 2024 Debrief Monitor  v0.1", 
        shiny::tags$br(), 
        shiny::tags$br(), 
...
ui_new <-   
    shiny::shinyUI(
        navbarPage(
          title = "MPIAE Retreat 2024 Debrief Monitor", 
...
server <- function(input, output, session) {
  ...
  output$corr_tab <- renderTable({
    ...
    output$download_report_docx <- downloadHandler(
      filename =  "retreat2024_retreat.docx",
      content = function(file) {
        file.copy("retreat2024_report.docx", file, overwrite = TRUE)
      }
    )
    output$download_report_pdf <- downloadHandler(
      filename =  "retreat2024_retreat.pdf",
      content = function(file) {
        file.copy("retreat2024_report.pdf", file, overwrite = TRUE)
      }
    )
